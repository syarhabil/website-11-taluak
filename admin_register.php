...
try {
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Periksa apakah username atau NIP sudah ada
        $check_sql = "SELECT * FROM admins WHERE username = ? OR nip = ?";
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                if ($row['username'] == $username) {
                    $error_message = "Username sudah ada, ganti dengan yang lain.";
                }
                if ($row['nip'] == $nip) {
                    $error_message .= "<br>NIP sudah ada, ganti dengan yang lain.";
                }
            }
        } else {
            $sql = "INSERT INTO admins (full_name, gender, dob, address, email, phone, nip, position, username, password, profile_photo, security_question, security_answer, is_verified) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0)";
            ...
            if ($stmt->execute()) {
                if (move_uploaded_file($_FILES['foto_profil']['tmp_name'], $target)) {
                    $success_message = "Akun Anda sedang diverifikasi. Silakan tunggu persetujuan.";
                    header("Location: admin_login.php?success_message=" . urlencode($success_message));
                    exit();
                } else {
...
<head>
    ...
    <title>Daftar Admin - SDN 11 Taluak</title>
                            ...
                            <div class="card shadow-lg border-0 rounded-lg mt-5">
                                ...
                                <div class="card-body">
                                    ...
                                    <form method="POST" action="admin_register.php" enctype="multipart/form-data">
                                        ...
                                        <div class="form-floating mb-3">
                                            <input class="form-control" id="jabatan" name="jabatan" type="text" placeholder="Jabatan" value="<?php echo isset($_POST['jabatan']) ? $_POST['jabatan'] : ''; ?>" required />
                                ...
                                <div class="card-footer text-center py-3">
                                    <div class="small"><a href="admin_login.php">Sudah punya akun? Masuk di sini</a></div>
                                </div>
                            </div>
                        </div>
                    </div>
